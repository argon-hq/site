...
describe("persistCandidate", () => {
  ...
  it("below the cutoff only marks the link as seen, without reading the page", async () => {
    ...
    expect((await Effect.runPromise(persistCandidate({ ...candidate, score: 2 }, c, read))).outcome).toBe(
      "below_cutoff",
    );
    expect(c.mocks.seenUrl.upsert).toHaveBeenCalledOnce();
  ...
  it("rejects other domains before touching the database, and old or unreadable pages after marking them seen", async () => {
    const c = ctx();
    expect(
      await Effect.runPromise(persistCandidate({ ...candidate, url: "https://g1.globo.com/x" }, c, page(null))),
    ).toMatchObject({ outcome: "rejected", reason: "domain not allowed" });
    expect(c.mocks.seenUrl.upsert).not.toHaveBeenCalled();
    expect(await Effect.runPromise(persistCandidate(candidate, c, page("2026-09-10T00:00:00Z")))).toMatchObject({
      outcome: "rejected",
      reason: "outside window",
    });
    const unreadable: ReadPage = (url) => Effect.fail(new PageUnreadable({ url, reason: "no readable text" }));
    expect(await Effect.runPromise(persistCandidate(candidate, c, unreadable))).toMatchObject({
      outcome: "rejected",
      reason: "PageUnreadable: no readable text",
    });
    expect(c.mocks.article.create).not.toHaveBeenCalled();
  });
  ...
  it("stores the page's own canonical url and marks both links as seen", async () => {
    ...
    const read = page(
      ...
      "https://www.valor.globo.com/empresas/noticia/2026/09/18/x.ghtml?ref=home",
    );
    const out = await Effect.runPromise(
      persistCandidate({ ...candidate, url: "https://valor.globo.com/amp/empresas/x.ghtml" }, c, read),
    );
    expect(out).toEqual({
      outcome: "saved",
      url: "https://valor.globo.com/empresas/noticia/2026/09/18/x.ghtml",
    ...
    expect(c.mocks.article.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ canonicalUrl: "https://valor.globo.com/empresas/noticia/2026/09/18/x.ghtml" }),
      }),
    );
  });
  ...
  it("rejects a canonical url that leaves the allowlist", async () => {
    ...
    expect(await Effect.runPromise(persistCandidate(candidate, c, read))).toMatchObject({
      outcome: "rejected",
      reason: "canonical url outside the allowlist",
    });
    expect(c.mocks.article.create).not.toHaveBeenCalled();
  });
});
...
describe("dedupeCandidates", () => {
  it("keeps the first of two candidates with the same canonical url", () => {
    ...
    const twin = {
      ...candidate,
      url: "https://www.valor.globo.com/empresas/noticia/2026/09/18/x.ghtml?utm_source=x",
      score: 3,
    };
