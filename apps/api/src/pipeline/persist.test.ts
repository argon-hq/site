...
function ctx(existing = false) {
  const mocks = {
    seenUrl: { upsert: vi.fn(async () => ({})) },
    article: { findUnique: vi.fn(async () => (existing ? { id: "1" } : null)), create: vi.fn(async () => ({ id: "2" })) },
  };
  const context: PersistContext = { prisma: mocks as unknown as PrismaClient, since: new Date("2026-09-17T00:00:00Z"), cutoff: 3 };
  return Object.assign(context, { mocks });
}

const page = (publishedAt: string | null): ReadPage => (url) =>
  Effect.succeed({ canonicalUrl: url, originalTitle: "Título da página", extractedText: "texto", siteName: "Valor", publishedAt });
