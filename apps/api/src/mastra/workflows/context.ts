...
export type EditionSteps = {
  collect(now?: Date): Effect.Effect<CollectReport, { reason: string }>;
  write(now?: Date): Effect.Effect<WriteReport, { reason: string }>;
  build(now?: Date): Effect.Effect<BuildReport, { reason: string }>;
};
